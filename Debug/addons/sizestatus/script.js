...
if (window.Addon == 1) {
	Addons.SizeStatus = {
		...
		Show: async function (FV, Id) {
			...
			// FV.SyncGen is bumped whenever SyncItems (TE.cpp) reconciled the view with
			// the disk, so a file growing in place invalidates the cache even though the
			// item count is unchanged.
			const SessionId = await api.HashData(nCount ? await ExtractMacro(te, '%Selected%') : BuildPath(await pid.Path, await FV.FilterView, nAll.toString(), String(await FV.SyncGen || 0)), 8);
